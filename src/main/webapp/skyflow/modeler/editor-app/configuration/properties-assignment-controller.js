...
angular.module('flowableModeler').controller('FlowableAssignmentPopupCtrl',
    ...
    $scope.assignees=[
        {value:null,title:"空"},
        {value:"${selectUserStrategy.setAssigneeToDeptLeader(execution)}",title:"实际申请人的部门领导"} ,
        {value:"wangxiaogang",title:"总裁"}
    ];

    $scope.candidateUsers=[
        {value:null,title:"空"},
        {value:"${selectUserStrategy.setAssigneeToDeptLeader(execution)}",title:"实际申请人的部门领导"} ,
        {value:"zhangsan,lisi",title:"候选人员组1"},
        {value:"wangwu,zhaoliu",title:"候选人员组2"},
        {value:"zhangsan",title:"候选人员组3"}
    ];
    ...
    if ($scope.assignment.candidateUsers && $scope.assignment.candidateUsers.length > 0) {
        ...
        for (var i = 0; i < $scope.assignment.candidateUsers.length; i++) {
            if($scope.popup.assignmentObject.static.candidateUsers){
                $scope.popup.assignmentObject.static.candidateUsers += ","+$scope.assignment.candidateUsers[i].value;
            }else{
                $scope.popup.assignmentObject.static.candidateUsers += $scope.assignment.candidateUsers[i].value;
            }
        }
    ...
    $scope.save = function () {
        ...
        if ('static' === $scope.popup.assignmentObject.type) { // IDM
            ...
            var arr = $scope.popup.assignmentObject.static.candidateUsers?$scope.popup.assignmentObject.static.candidateUsers.split(","):[];
            var users = [];
            for(var i=0;i<arr.length;i++){
                users.push({value:arr[i]});
            }
