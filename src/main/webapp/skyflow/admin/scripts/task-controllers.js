...
flowableAdminApp.controller('EditTaskModalInstanceCtrl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'task', function ($rootScope, $scope, $modalInstance, $http, task) {
      ...
      function dateTimeFormat(date) {
          var d = new Date(date);
          var dueDate= d.getFullYear()+"-"+(d.getMonth()+1)+"-"+d.getDate()+" "+d.getHours()+":"+d.getMinutes()+":"+d.getSeconds() ;
          return dueDate;
      }
      //endregion
	  ...
	  $scope.ok = function () {
		  ...
		  if($scope.model.dueDate){
              $scope.model.dueDate=dateTimeFormat($scope.model.dueDate);
          }
          //endregion fxx20171018
