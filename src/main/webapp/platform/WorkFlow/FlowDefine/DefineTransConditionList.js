...
var transitiondefineid = 0; //当前流向模型id
...
function addGroupNode(parentNode) {
    ...
    $('#transitionConditionList').treegrid('append', { parent: parentNode.id,
        data: [{ id: tempId,
            ...
            relation: '',
            isgroup: 1,
            transitiondefineid: transitiondefineid,
...
function addGroupEditor(parentNode) {
    ...
    if (!parentNode.tablename) {
        ...
        GlobalTools.ajax({
            async: false,
            dataType: "json",
            url: rootPath + "Sys_def_activityform/LoadTailorformIds.do",
...
function saveCondition(flag) {//flag表示保存成功后   1:需要添加分组节点   0 :需要添加条件节点  2：不需要添加节点
    ...
    var params = { transitiondefineid: transitiondefineid, changeDatas: JSON2.stringify(changeDatas), allDatas: JSON2.stringify(allDatas) };
    if (flag != 2) {//表示要新增分组节点或者条件节点
        changeDatas = [currentNode]; //只保存新增的父节点数据
        params = { transitiondefineid: transitiondefineid, parentidTemp: currentNode.id, changeDatas: JSON2.stringify(changeDatas), allDatas: JSON2.stringify(allDatas) };
    }
    GlobalTools.ajax({
        dataType: "json",
        url: rootPath + "Sys_def_transcondition/SaveTranscondition.do",
        data: params,
        success: function (data, msg) {
            if (flag != 2) {//表示要新增分组节点或者条件节点
...
function destroyCondition() {
    ...
    $.messager.confirm('操作确认', '确定要删除这些条件吗？  删除后将无法恢复。', function (result) {
        if (result) {
            ...
            GlobalTools.ajax({
                dataType: "json",
                url: rootPath + "Sys_def_transcondition/DestroyList.do",
                data: { transitiondefineid: transitiondefineid, ids: destroyIdArray.join(',') },
                success: function (data, msg) {
                    $('#transitionConditionList').treegrid('reload');
                }
            });
