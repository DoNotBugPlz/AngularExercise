...
(function ($) {
	...
	function initLayout(target) {
		...
		var center = $('<div id="centerPanel"/>').attr({
				'border' : false,
				'region' : 'center'
			}).addClass('app-wall').appendTo(jqTarget);
		
		var  contentLayOut =  $('<div  id="contentLayOut"></div>').attr({ //嵌套layout
				'fit' : true
				
				
			}).addClass('app-wall').css("width",'100%').css("height","100%").appendTo(center);
		
		
		var fmenuDiv =  $('<div id="left"></div>').attr({ //1级菜单
				'border' : false,
				'region' : 'west'

			}).addClass('app-wall').css("width",'200px').css("filter","alpha(enabled=true,opacity=50%)").appendTo(contentLayOut);
		
		
		var smenuDiv =  $('<div id="center"/>').attr({ //2级菜单
				'border' : false,
				'region' : 'center',
				"background-color" : "transparent" 
				
			}).addClass('app-wall').appendTo(contentLayOut);
	...
	function initDeskTop(target) {
		var jqTarget = $(target);
		var opts = $.data(target, 'app').options;
		/**var wall = jqTarget.layout('panel', 'center').layout('panel'); //桌面对象*/
		var wall = $("#contentLayOut").layout('panel','center');  //二级菜单
		var appContainer = $('<ul/>').addClass('app-container'); //app容器 
		
		var firstwall= $("#contentLayOut").layout('panel','west');  //1级菜单
		var firstappContainer=$('<ul/>').addClass('app-container'); //app容器
		appContainer.appendTo(wall);
		firstappContainer.appendTo(firstwall);
		
		$.data(target, 'app')['appContainer'] = appContainer;
		$.data(target, 'app')['firstappContainer'] = firstappContainer;
		...
		if (opts.loadUrl.app && !loaded) {
			$.ajax({
				...
				success : function (resp) {
					initleftDiv(target,resp); //初始化一级菜单
				/*	initApp(target, resp);初始化二级菜单*/
				},
	...
	function  initleftDiv(target,apps){
		
		
		var jqTarget = $(target);
		var opts = $.data(target, 'app').options;
		
		var wall = $("#contentLayOut").layout('panel', 'west'); //桌面对象
		
		var appContainer = jqTarget.data().app.firstappContainer;
		
		
		
		
		var lines = Math.floor((wall.height() - 20) / (opts.firsticonSize + 45)); //可显示行数
		
		var columns = Math.floor((wall.width() - 20) / (opts.firsticonSize + 45)); //可显示列数
		...
		for (var i = 0;i<apps.length;i++) {
			...
			var icon = $('<img/>').height(opts.iconSize).width(opts.iconSize).attr('src', rootPath+app.txtExtchar1).appendTo(appItem); //修改图标参数 zgy
			$("<br/>").appendTo(appItem)
			var text = $('<span/>').text(app.menuName).appendTo(appItem);//修改
			var em = $('<em/>').css({
					height : relSize,
					width : relSize
				}).appendTo(appItem);
			...
			if ($.browser.msie) { //兼容ie的hover
				...
				appItem.on('click', function () { //单击刷新右侧div(二级菜单)
					var id=$(this).attr("id");
					...
					$.data(target, 'app')[id]=resp; //菜单信息加入data缓存中 
	...
	function initApp(target, apps) {
		...
		for (var i = 0;i<apps.length;i++) {
			...
			if(opts.dragable){
				initAppDrag(target, appItem);//初始化app的拖拽事件  修改 添加控制参数  zgy
			}
	...
	function openApp(target, options) {
		...
		if (appOpt.pageUrl && !/^http/i.test(appOpt.pageUrl) && !appOpt.iframe) {
			
			config.href = rootPath+appOpt.pageUrl;
		}
		alert(config.href)
		...
		function appendToList(uuid, text) {
			...
			if ($('li[l_id="' + uuid + '"]', list).length) {
				...
				if (wrap.width() > taskList.width()) {
					...
					item.hide();   //超出范围  将当前li隐藏  add by zgy
					$('div[class^="app-scroll-right"]', taskList).show();
				...
				$('div[class^="app-scroll-"]').unbind("click").click(function(){//点击左右三角框    add by zgy
					var className=$(this).attr("class");
				
					if(className=="app-scroll-left")
						{ 
							var lastViewable=list.children(":visible").last();
							
							var fstViewable=list.children(":visible").first();
							fstViewable.prev().show();
							lastViewable.hide();
							lastViewable.nextAll().hide();
						//将最后的一个可见和之前的给隐藏掉//并把它之前的全部show
							
							if(!fstViewable.prev().prev("li").attr("l_id"))
								{
									$('div[class=app-scroll-left]').hide();
								
								}
							
							$('div[class=app-scroll-right]').show();
						
						}
					else if(className=="app-scroll-right"){
						var fstViewable=list.children(":visible").first();
						var lastViewable=list.children(":visible").last();
						
						fstViewable.hide();  
						fstViewable.prevAll().hide(); 
						lastViewable.next().show();
						//将第一个可见的隐藏掉，并把它之后的全部show
						
						if(!lastViewable.next().next("li").attr("l_id"))
							{
									$('div[class=app-scroll-right]').hide();
								
							}
							
						
						$('div[class=app-scroll-left]').show();
					}
					var chidls=list.children(":visible");
					var width=0;
					for(var i=0;i<chidls.length;i++){
					
						width=chidls.eq(i).outerWidth()+width;
		...
		function removeListItem(uuid) {
			...
			wrap.width(wrap.width() - (item.outerWidth() +10));
	...
	$.fn.app.defaults = {
		...
		firsticonSize:60 ,//1级菜单图标大小
		iconSize : 50, //app图标大小  //二级菜单
