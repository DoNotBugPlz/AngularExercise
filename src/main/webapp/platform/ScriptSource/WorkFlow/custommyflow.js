(function ($) {
    ...
    myflow.config = {
        ...
        path: {// 路径转换
            ...
            props: {
                id: { value: '' },
                startcustomactivityid: { value: '' },
                endcustomactivityid: { value: '' }
            }
        },
        tools: {// 工具栏
            attr: {
    ...
    myflow.rect = function (o, r) {
        ...
        // 事件处理--------------------------------
        $([_rect.node, _text.node, _name.node, _img.node]).unbind('click').bind('click',
				function () {
				    if (!myflow.config.editable)
				        return;
				    ...
				    switch (mod) {
				        case 'pointer': //选择
				            var nodeData = eval("(" + _this.toJson(_this.getId()) + ")"); //节点的数据json
				            //开始 节点不可编辑
				            if (nodeData && nodeData.type != "start") {
				                //debugger;
				                whichNode = nodeData;
				                addCustomAcitvityEditPages();
				            }
				            else {
				                $("#editPanel").html("");
				            }
				        ...
				        case 'path': //转换 添加连线状态
				            ...
				            if (pre && pre.getId() != _id
				                ...
				                //r:连线的开始节点； u：连线的结束节点
				                var r = $(_r).data("currNode");
				                if (r != null) {
				                    if (_o.type == "start") {
				                        GlobalTools.tip("不能指向开始步骤！");
				                        $(_r).trigger('click', _this);
				                        $(_r).data('currNode', _this);
				                        return;
				                    }

				                    var startNodeJson = eval("(" + r.toJson() + ")"); //通过节点的Json来找需要的信息
				                    if ((!startNodeJson.iscurrentrect)&&(!startNodeJson.editable)) {
				                        GlobalTools.tip("已执行过的节点不可指向其他节点！");
				                        $(_r).trigger('click', _this);
				                        $(_r).data('currNode', _this);
				                        return;
				                    }

				                    var endNodeJson = eval("(" + _this.toJson() + ")");
				                    if (startNodeJson.props.id.value == endNodeJson.props.id.value) {
				                        GlobalTools.tip("不能选择同一个节点！");
				                        $(_r).trigger('click', _this);
				                        $(_r).data('currNode', _this);
				                        return;
				                    }
				                   
				                    if(startNodeJson.type=="start"){ 
				                    	var isCanAdd=false;
				                    	 GlobalTools.ajax({
				                    		 url:rootPath+"Sys_custom_transition/IsCanAddPathFromStartNode.do",
				                    		 async:false,
				                    		 data:{startcustomactivityid:startNodeJson.props.id.value},
        ...
        // 函数----------------
        // 转化json字串
        this.toJson = function (nodeid) {
            var data = "{node:'" + nodeid + "',type:'" + _o.type + "',text:{value:'"
					+ _text.attr('text') + "'}, attr:{ x:"
					+ Math.round(_rect.attr('x')) + ", y:"
					+ Math.round(_rect.attr('y')) + ", width:"
					+ Math.round(_rect.attr('width')) + ", height:"
					+ Math.round(_rect.attr('height')) + "}, editable:" + _o.editable + ",iscurrentrect:"+_o.iscurrentrect+",props:{";
    ...
    myflow.path = function (o, r, from, to) {
        var _this = this, _r = r, _o = $.extend(true, {}, myflow.config.path), _path, _arrow, _text, _textPos = _o.textPos, _ox, _oy, _from = from, _to = to, _id = 'path'
        ...
        // 事件处理--------------------
        $([_path.node, _arrow.node, _text.node]).unbind('click').bind('click', function () {
            if (!myflow.config.editable)
                return;
            $(_r).trigger('click', _this);
            $(_r).data('currNode', _this);
            var mod = $(_r).data('mod');
            if (mod == "pointer") {
                whichPath = eval("(" + _this.toJson() + ")");
                addCustomTransitionEditPage();
            }
            return false;
        });
    ...
    myflow.init = function (c, o) {
        ...
        */
        $(document).keydown(function (arg) {
            if (!myflow.config.editable)
                return;
            if (arg.keyCode == 46 && isSelected == true) {
                var c = $(_r).data('currNode');
                if (c) {
                    var rectPathJson = eval("(" + c.toJson() + ")");
                    var rectOrPath = c.getId().substring(0, 4);
                    if (rectOrPath == 'rect') {
                        if (rectPathJson.type == "start" || rectPathJson.type == "end") {
                            GlobalTools.tip("开始节点不能删除！");
                            return;
                        }
                        if (!rectPathJson.editable) {
                            GlobalTools.tip("该节点已经执行过,不能删除！");
                            return;
                    ...
                    var rectPathId = rectPathJson.props.id.value;
                    var rectPathText = rectPathJson.text.value;
                    if (rectOrPath == 'rect') {
                        //删除某流程节点时，需对应删除与该节点关联的连线
                        GlobalTools.ajax({
                            async: false,
                            type: "POST",
                            url: rootPath + "Sys_custom_activity/DeleteCustomActivity.do",
                            data: { id: rectPathId },
                            beforeSend: function () {
                                GlobalTools.showLoading("正在删除..........");
                            },
                            complete: function () {
                            ...
                            success: function (data) {
                                ...
                                GlobalTools.tip("节点（" + rectPathText + "）删除成功！");
                                $("#editPanel").html("");
                            },
                            error: function () {
                                GlobalTools.tip("节点（" + rectPathText + "）删除失败！");
                            }
                        });
                        $(_r).trigger('removerect', c);
                        $(_r).removeData('currNode');
                    } else if (rectOrPath == 'path') {
                        //删除某流程节点时，需对应删除与该节点关联的连线
                        GlobalTools.ajax({
                            async: false,
                            type: "POST",
                            url: rootPath + "Sys_custom_transition/DeleteCustomPath.do",
                            data: { id: rectPathId },
                            beforeSend: function () {
                                GlobalTools.showLoading("正在删除..........");
                            },
                            ...
                            success: function () {
                                ...
                                $(_r).removeData('currNode');
                                GlobalTools.tip('连线（' + rectPathText + '）删除成功！');
                                $("#editPanel").html("");
                            },
                            error: function () {
                                GlobalTools.tip('连线（' + rectPathText + '）删除失败！');
                            }
                        });
                    }

                    //                    rectInfos = getRectJsonStr();
                    //                    pathInfos = getRectJsonStr();
                    //                    loadWorkFlowMap();
                    //                    $(_r).removeData('currNode');
                }
            }
        ...
        $(_r).bind('addrect', function (e, type, o) {
            ...
            var newAddRect = rect.getId();
            _states[newAddRect] = rect;
            var rectData = eval("(" + rect.toJson() + ")");
            var tempPathStr = getPathJsonStr();

            GlobalTools.ajax({
                cache: false,
                async: false,
                url: rootPath + "Sys_custom_activity/SaveRectByDrop.do",
                data: { currentinsactivityid: insactivityid, type: rectData.type, x: rectData.attr.x, y: rectData.attr.y, text: rectData.text.value },
                success: function (data) {
                    var tempRectStr = "";
                    var tempRectData;
                    var propsId;
                    for (var index in _states) {
                        if (_states[index] != null) {
                            tempRectData = eval("(" + _states[index].toJson(index) + ")");
                            propsId = tempRectData.props.id.value;

                            if (index == newAddRect)//新添加的节点的id
                            {
                                whichNode = tempRectData;
                                whichNode.props.id.value = data.sys_custom_activity.id;
                                /// whichNode.props.id.value = data.id;
                                propsId = data.sys_custom_activity.id;
                            }
                            tempRectStr += index + ":{type:'" + tempRectData.type + "',text:{text:'" + tempRectData.text.value + "'}, attr:{ x:" + tempRectData.attr.x + ", y:" + tempRectData.attr.y + ", width:" + tempRectData.attr.width + ", height:" + tempRectData.attr.height + "},editable:"+tempRectData.editable+",iscurrentrect:"+tempRectData.iscurrentrect+",props:{id:{value:'" + propsId + "'}}},";
                        }
                    }
                    if (tempRectStr.substring(tempRectStr.length - 1, tempRectStr.length) == ',') {
                        customRectInfos = tempRectStr.substring(0, tempRectStr.length - 1);
                        customPathInfos = tempPathStr;
        ...
        var getRectJsonStr = function () {
            var tempRectStr = "";
            var tempRectData;
            for (var index in _states) {
                if (_states[index] != null) {
                    tempRectData = eval("(" + _states[index].toJson(index) + ")");
                    tempRectStr += index + ":{type:'" + tempRectData.type + "',text:{text:'" + tempRectData.text.value + "'}, attr:{ x:" + tempRectData.attr.x + ", y:" + tempRectData.attr.y + ", width:" + tempRectData.attr.width + ", height:" + tempRectData.attr.height + "}, props:{id:{value:'" + tempRectData.props.id.value + "'}}},";
                }
            }
            if (tempRectStr.substring(tempRectStr.length - 1, tempRectStr.length) == ',') {
        ...
        var getPathJsonStr = function () {
            var tempPathStr = "";
            var tempPathData;
            for (var index in _paths) {
                if (_paths[index] != null) {
                    tempPathData = eval("(" + _paths[index].toJson() + ")");
                    tempPathStr += index + ":{from:'" + tempPathData.from + "',to:'" + tempPathData.to + "', dots:[" + tempPathData.dots + "],text:{text:'" + tempPathData.text.value + "'},textPos:{x:" + tempPathData.textPos.x + ",y:" + tempPathData.textPos.y + "}, props:{id:{value:'" + tempPathData.props.id.value + "'},startcustomactivityid:{value:'" + tempPathData.props.startcustomactivityid.value + "'},endcustomactivityid:{value:'" + tempPathData.props.endcustomactivityid.value + "'}}},";
                }
            }
        ...
        var addpathHandler = function (e, from, to) {
            ...
            var newAddPath = path.getId();
            _paths[newAddPath] = path;
            var pathData = eval("(" + path.toJson() + ")");
            var fromRect = eval("(" + _states[pathData.from].toJson() + ")");
            var toRect = eval("(" + _states[pathData.to].toJson() + ")");
            var tempRectStr = getRectJsonStr();
            GlobalTools.ajax({
                cache: false,
                async: false,
                url: rootPath + "Sys_custom_transition/SavePathByDrop.do",
                data: { currentinsactivityid: insactivityid, text: pathData.text.value, startcustomactivityid: fromRect.props.id.value, endcustomactivityid: toRect.props.id.value },
                success: function (data) {
                    var tempPathStr = "";
                    var tempPathData;
                    var propsId;
                    for (var index in _paths) {
                        if (_paths[index] != null) {
                            tempPathData = eval("(" + _paths[index].toJson() + ")");
                            propsId = tempPathData.props.id.value;
                            if (index == newAddPath)//新添加的节点的id
                            {
                                whichPath = tempPathData;
                                whichPath.props.id.value = data.id;
                                propsId = data.id;
                            }
                            tempPathStr += index + ":{from:'" + tempPathData.from + "',to:'" + tempPathData.to + "', dots:[" + tempPathData.dots + "],text:{text:'" + tempPathData.text.value + "'},textPos:{x:" + tempPathData.textPos.x + ",y:" + tempPathData.textPos.y + "}, props:{id:{value:'" + tempPathData.props.id.value + "'},startcustomactivityid:{value:'" + tempPathData.props.startcustomactivityid.value + "'},endcustomactivityid:{value:'" + tempPathData.props.endcustomactivityid.value + "'}}},";
                        }
                    ...
                    if (tempPathStr.substring(tempPathStr.length - 1, tempPathStr.length) == ',') {
                        customPathInfos = tempPathStr.substring(0, tempPathStr.length - 1);
                        customRectInfos = tempRectStr;
                        loadCustomWorkFlowMap();
                        rectPathIsFlowCustom = true;
        ...
        if (myflow.config.editable) {
            ...
            $('#myflow_tools .state').each(function () {
                $(this).draggable({
                    ...
                    onStopDrag: function (e) {
                        if (Math.abs(e.screenX - dragStartX) > 70 || Math.abs(e.screenY - dragStartY) > 20) {
                            var d = e.data;
                            $(_r).trigger('addrect', [$(sourceElement).attr('type'), {
                                attr: {
                                    x: d.left + $("#divMyflow").scrollLeft(),
                                    y: d.top + $("#divMyflow").scrollTop()
                                }
                            }]);
                        }
                    }
            ...
            $('#myflow_save').unbind('click').bind('click', function () {// 保存
                var data = '{states:[';
                for (var k in _states) {
                    if (_states[k]) {
                        data += _states[k].toJson(_states[k].getId()) + ',';
                    }
                }
                if (data.substring(data.length - 1, data.length) == ',')
                    data = data.substring(0, data.length - 1);
                ...
                $.ajax({
                    ...
                    async: false,
                    type: "POST",
                    url: rootPath + "Sys_custom_activity/SaveCustomWorkFlowDesign.do",
                    data: { currentinsactivityid: insactivityid, info: data },
                    success: function (data) {
                        if (data.iserror) {
                            GlobalTools.tip("流程图保存失败");
                        }
                        else {
                            GlobalTools.tip("流程图保存成功");
                        }
                    }
                });
        ...
        if (hr.rects.length || ar.rects.length) {
            ...
            for (var pid in _paths) {// 先组织MAP
                ...
                var nodeName = "rect" + nodeObj.props.id.value;
