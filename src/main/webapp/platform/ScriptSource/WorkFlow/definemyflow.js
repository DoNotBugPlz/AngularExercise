(function ($) {
    ...
    myflow.config = {
        ...
        path: {// 路径转换
            ...
            props: {
                id: { value: '' },
                startactivitydefineid: { value: '' },
                endactivitydefineid: { value: '' }
            }
        },
        tools: {// 工具栏
            attr: {
            ...
            function () {
                ...
                switch (mod) {
                    case 'pointer': //选择
                        ...
                        //结束 节点不可编辑
                        if (nodeData && nodeData.type != "end") {
                            //debugger;
                            whichNode = nodeData;
                            addDefAcitivtyEditPages(nodeData.type);
                        }
                        else {
                            $("#editPanel").html("");
                        }
                    ...
                    case 'path': //转换 添加连线状态
                        ...
                        if (pre && pre.getId() != _id
                            ...
                            if (r != null) {
                                ...
                                if (startNodeJson.type == "end") {
                                    GlobalTools.tip("办结节点不可指向其他节点！");
                                    $(_r).trigger('click', _this);
                                    $(_r).data('currNode', _this);
                                    return;
                                }
                                ...
                                if (startNodeJson.type == "scan" && endNodeJson.type != "scan") {
                                    GlobalTools.tip("传阅节点只能指向传阅节点！");
                                    $(_r).trigger('click', null);
                                    $(_r).data('currNode', null);
                                    return;
                                }
                                if (startNodeJson.type == "fork") {
                                    if (endNodeJson.type == "fork" || endNodeJson.type == "join") {
                                        GlobalTools.tip("分支、合并节点不能互连！");
                                        $(_r).trigger('click', _this);
                                        $(_r).data('currNode', _this);
                                        return;
                                    }
                                }
                                if (startNodeJson.type == "join") {
                                    if (endNodeJson.type == "fork" || endNodeJson.type == "join") {
                                        GlobalTools.tip("分支、合并节点不能互连！");
                                        $(_r).trigger('click', _this);
                                        $(_r).data('currNode', _this);
    ...
    myflow.init = function (c, o) {
            ...
            if (c) {
                ...
                if (c.getId().substring(0, 4) == 'rect') {
                    ...
                    GlobalTools.ajax({
                        async: false,
                        type: "POST",
                        url: rootPath + "Sys_def_activity/DeleteActivity.do",
                        data: { id: rectPathId },
                        beforeSend: function () {
                    ...
                    GlobalTools.ajax({
                        async: false,
                        type: "POST",
                        url: rootPath + "Sys_def_transition/DeletePath.do",
                        data: { id: rectPathId },
                        beforeSend: function () {
                            GlobalTools.showLoading("正在删除..........");
                        },
        ...
        $(_r).bind('addrect', function (e, type, o) {
            ...
            GlobalTools.ajax({
                cache: false,
                async: false,
                url: rootPath + "Sys_def_activity/SaveRectByDrop.do",
                data: { processid: processid, type: rectData.type, x: rectData.attr.x, y: rectData.attr.y, text: rectData.text.value },
                success: function (data) {
                    var tempRectStr = "";
                    var tempRectData;
                    var propsId;
                    for (var index in _states) {
                        if (_states[index] != null) {
                            ...
                            tempRectStr += index + ":{type:'" + tempRectData.type + "',text:{text:'" + tempRectData.text.value + "'}, attr:{ x:" + tempRectData.attr.x + ", y:" + tempRectData.attr.y + ", width:" + tempRectData.attr.width + ", height:" + tempRectData.attr.height + "}, props:{id:{value:'" + propsId + "'}}},";
                        }
                    }
                    if (tempRectStr.substring(tempRectStr.length - 1, tempRectStr.length) == ',') {
                        rectInfos = tempRectStr.substring(0, tempRectStr.length - 1);
                        pathInfos = tempPathStr;
                        loadWorkFlowMap();
                        rectPathIsFlowCustom = true;

                        addDefAcitivtyEditPages();
                    }
                }
            });
        });
        ...
        var addpathHandler = function (e, from, to) {
            ...
            GlobalTools.ajax({
                cache: false,
                async: false,
                url: rootPath + "Sys_def_transition/SavePathByDrop.do",
                data: { processid: processid, text: pathData.text.value, startactivitydefineid: fromRect.props.id.value, endactivitydefineid: toRect.props.id.value },
                success: function (data) {
                    var tempPathStr = "";
                    var tempPathData;
                    var propsId;
                    ...
                    if (tempPathStr.substring(tempPathStr.length - 1, tempPathStr.length) == ',') {
                        pathInfos = tempPathStr.substring(0, tempPathStr.length - 1);
                        rectInfos = tempRectStr;
                        loadWorkFlowMap();
                        rectPathIsFlowCustom = true;
        ...
        if (myflow.config.editable) {
            ...
            $("#myflow_info").unbind('click').bind('click', function () {
                addDefProcessEditPage();
            });
            ...
            $('#myflow_save').unbind('click').bind('click', function () {// 保存
                ...
                $.ajax({
                    ...
                    async: false,
                    type: "POST",
                    url: rootPath + "Sys_def_process/SaveDefineWorkFlowDesign.do",
                    data: { processid: processid, info: data },
                    success: function (data) {
                        if (data.iserror) {
