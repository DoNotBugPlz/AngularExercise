(function ($) {
        ...
        $([_rect.node, _text.node, _name.node, _img.node]).unbind('click').bind('click',
				function () {
				    ...
				    if(mod=="point"){
                        ...
                        //结束 节点不可编辑
                        if (nodeData && nodeData.type != "end" && nodeData.type !="fork" && nodeData.type !="join") {
                            showDivInsActInfo(nodeData.props.id.value,nodeData.text.value);
                        }
                    }
    ...
    myflow.init = function (c, o) {
        ...
        if (hr.rects.length || ar.rects.length) {
            ...
            //增加type（节点类型），可能会需要根据type来筛选节点
            /*var objs = o.restore.states;
            for(var i in objs){
                if(rmap[i]){
                    rmap[i].type = objs[i].type;
                }
            }*/

            //从流程图节点中中识别出历史节点，并赋予历史节点状态
