...
//            rootPath: "http://localhost:8080/tlgada/", //只提供给loadServerDocument用
            ...
            postFile: "OfficeFile", //文档内容以PostFile发送到服务器时使用的名称
            ...
            loadDocumentActionPath: "platform/ScriptSource/NTKOWebOffice/Template.doc", //下载Office内容的服务器请求Url地址
        ...
        loadServerDocument: function (options, callback) {
            ...
            try {
                ...
                //WebOfficeOCX.OpenFromURL("http://32.1.2.61:8080/tlgada/Common/DownLoadTemplateFile.do?templateName=beijiqian3.xls", doctype);
               //WebOfficeOCX.OpenFromURL("http://32.1.2.61:8080/tlgada/tlgada/htmlinit/print/template/beijiqian3.xls", doctype);
                WebOfficeOCX.BeginOpenFromURL(opt.rootPath + opt.loadDocumentActionPath,doctype);
               // WebOfficeOCX.OpenFromURL(opt.rootPath + opt.loadDocumentActionPath,,doctype);
...
//            alert(opt.saveDocumentActionPath+"\n"+opt.postFile+"\n"+opt.postData+"\n"+opt.fileName+"\n"+opt.postFormId);
        ...
        insertBookMark: function (bookMarkName, bookMarkValue) {
            try {
                ...
                GlobalTools.showError("插入书签错误：" + Error.Description);
            }
        },
        //设置书签内容
        ...
        deleteBookMark: function (bookMarkName) {
            try {
                ...
                GlobalTools.showError("删除书签错误：" + Error.Description);
            }
        },
        //通过服务器文档内容替换书签
                ...
                for(i=1,scount=docMarks.Count;i<=scount;i++){
                   ...
                   if(item){
                       if (item.bookmarkname.toLowerCase() == "content") {
                           if (!opt.coverRedHeadPath || opt.coverRedHeadPath == "") { GlobalTools.showError("书签地址无效。"); return; }
...
/*
                ...
                GlobalTools.showError("替换书签错误：" + Error.Description);
            }
        },
