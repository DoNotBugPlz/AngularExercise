(function () {
    ...
    angular
        ...
        .controller('automaticDetailCtrl', automaticDetailCtrl);

    automaticDetailCtrl.$inject = ['localStorageService', '$scope', 'automaticdetailservice', 'ngDialog', 'coreService'];

    /* @ngInject */
    function automaticDetailCtrl(localStorageService, $scope, automaticdetailservice, ngDialog, coreService) {
        var vm = this;
        window.a = vm;
        window.b = $scope;
        ...
        function setMatter(response) {
            ...
            for( var i=0;i<response.data.monitor_greats.length;i++){
                ...
                vm.goodsLis.push({good_id:'',price_trend_id:'',orginal_price:orginal_price,cunrrent_price:cunrrent_price,change_reaon:change_reaon,price_readjust:price_readjust});
            }
        }
            ...
            if(goods==''){
                ...
                if(isNaN(law_phone) ||  isNaN(contacts_phone)){
                    ...
                    if ($scope.matter_form.$valid) {
                        ...
                        automaticdetailservice.saveMatter(params)
                            .then(saveSmsSuccess)
                    } else {
        ...
        function backUsers(value) {
            ...
            for(var i=0;i<value.length;i++){
                if(value[i]!=undefined){
                    ...
                    vm.goodsLis.push({name:value[i].name,good_id:value[i].goods_code,guige:value[i].spec,unit:value[i].measurement_unit,price_trend_id:'',orginal_price:'',cunrrent_price:'',change_reaon:'',price_readjust:''})
        ...
        function loadMatter() {
            ...
            automaticdetailservice.loadMatter(params)
                .then(setMatter);
        }
