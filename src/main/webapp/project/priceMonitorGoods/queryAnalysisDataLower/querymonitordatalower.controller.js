...
(function () {
    ...
    angular
        ...
        .controller('QuerymonitordatalowerCtrl', QuerymonitordatalowerCtrl);
    QuerymonitordatalowerCtrl.$inject = ['$scope', 'querymonitordatalowerService', 'pageInfDefault', 'ngDialog', '$sce', '$q', '_', '$window', 'SYSTEM'];

    /* @ngInject */
    function QuerymonitordatalowerCtrl($scope, querymonitordatalowerService, pageInfDefault, ngDialog, $sce, $q, _, $window, SYSTEM) {
        var vm = this;
        window.vm = vm;
        vm.title = 'QuerymonitordatalowerCtrl';
        ...
        function active() {
            ...
            $q.all([
                querymonitordatalowerService.loadList(vm.currentPageInf)
        ...
        function fineLoginAdminSuccess() {
            querymonitordatalowerService.searchDataByNoUrl()
            ...
            _.forEach(fineDatas, function (fineData) {
                jQuery.ajax({
                    ...
                    success: function (data) {
                        ...
                        querymonitordatalowerService.save(params).then(function (response) {
        ...
        function loadList(pageNumber, pageSize) {
            ...
            querymonitordatalowerService.loadList(params).then(loadListSuccess);
        }
        ...
        function addData(fineData) {
            /*  var xhr = new XMLHttpRequest();
              ...
              xhr.setRequestHeader("Access-Control-Allow-Origin", "http://32.1.0.33:37799");
            ...
            jQuery.ajax({
                type: 'POST',
                url: 'http://32.1.0.33:37799/WebReport/ReportServer?op=fr_bi&cmd=hangout_report_to_plate&_=0.06358614197274748',
                data: formData,
                beforeSend: function (request) {
                    request.setRequestHeader("Accept", "*/*");
                    request.setRequestHeader("Accept-Encoding", "gzip, deflate, br");
                    request.setRequestHeader("Accept-Language", "zh-CN,zh;q=0.8");
                    request.setRequestHeader("Access-Control-Allow-Origin", "http://32.1.0.33:37799");
                    ...
                    request.setRequestHeader("Referer", "http://32.1.0.33:37799/WebReport/ReportServer?op=fr_bi&cmd=hangout_report_to_plate");
            ...
            if (fineData.name) {
                querymonitordatalowerService.save(fineData)
        ...
        function editData(item) {
            ...
            opParamsNgDialog(title, params);
            return;
            var title = '修改查询';
            if (!item.build_url) {
                $q.all([
                    querymonitordatalowerService.login()
                    , querymonitordatalowerService.searchDataByNoUrl()
        ...
        function opParamsNgDialog(title, params) {
            ...
            ngDialog.open({
                title: title,
                template: "priceMonitorGoods/queryAnalysisDataLower/selectParams/querymonitordatalower.selectParams.html",
                height: height * 0.9 + 'px',
                width: width * 0.9 + 'px',
                controller: 'QuerymonitordatalowerSelectParamsCtrl as ps',
                data: params,
                resolve: {
                    loadEditFiles: function ($ocLazyLoad) {
                        return $ocLazyLoad.load([
                            './priceMonitorGoods/queryAnalysisDataLower/selectParams/querymonitordatalower.selectParams.controller.js',
                            './priceMonitorGoods/queryAnalysisDataLower/selectParams/querymonitordatalower.selectParams.service.js'
                        ]);
                    }
                },
                preCloseCallback: function () {
        ...
        function is_publicData(item) {
            ...
            if (item.data_type) {
                ...
                ngDialog.open({
                    title: '选择共享人',
                    template: "priceMonitorGoods/queryAnalysisDataLower/object_list/object_list.html",
                    height: '800px',
                    width: '800px',
                    ...
                    resolve: {
                        loadEditFiles: function ($ocLazyLoad) {
                            return $ocLazyLoad.load([
                                './priceMonitorGoods/queryAnalysisDataLower/object_list/object_list.controller.js',
                                './priceMonitorGoods/queryAnalysisDataLower/object_list/object_list.service.js',
                                'mCustomScrollbar'
                            ]);
                        }
                    },
        ...
        function cancelCollctData(item) {
            ...
            AppTools.confirm("确定收藏所选记录吗？", function () {
                querymonitordatalowerService.cancelCollctData({'ids': item.id}).then(function (response) {
                    if (response.data) {
                        loadList();
        ...
        function delData(item) {
            ...
            AppTools.confirm("确定删除所选记录吗？", function () {
                querymonitordatalowerService.delData({'ids': item.id}).then(function (response) {
                    if (response.data) {
                        loadList();
        ...
        function viewAllCollect() {
            ...
            ngDialog.open({
                title: '查看其它共享',
                template: "priceMonitorGoods/queryAnalysisDataLower/otherShare/querymonitordatalower.otherShare.html",
                height: height * 0.9 + 'px',
                width: width * 0.9 + 'px',
                controller: 'QuerymonitordatalowerOtherShareCtrl as os',
                // data: params,
                resolve: {
                    loadEditFiles: function ($ocLazyLoad) {
                        return $ocLazyLoad.load([
                            './priceMonitorGoods/queryAnalysisDataLower/otherShare/querymonitordatalower.otherShare.controller.js',
                            './priceMonitorGoods/queryAnalysisDataLower/otherShare/querymonitordatalower.otherShare.service.js'
                        ]);
                    }
                },
                preCloseCallback: function () {
