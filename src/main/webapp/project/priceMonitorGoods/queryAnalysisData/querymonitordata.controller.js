...
(function () {
    ...
    angular
        ...
        .controller('QuerymonitordataCtrl', QuerymonitordataCtrl);
    QuerymonitordataCtrl.$inject = ['$scope', 'querymonitordataService', 'pageInfDefault', 'ngDialog', '$sce', '$q', '_', '$window', 'SYSTEM'];

    /* @ngInject */
    function QuerymonitordataCtrl($scope, querymonitordataService, pageInfDefault, ngDialog, $sce, $q, _, $window, SYSTEM) {
        var vm = this;
        window.vm = vm;
        vm.title = 'QuerymonitordataCtrl';
        /* 配置类型为本级配置 */
        var config_type = 1;
        ...
        function active() {
            ...
            $q.all([
                querymonitordataService.loadList(vm.currentPageInf)
        ...
        function fineLoginAdminSuccess() {
            querymonitordataService.searchDataByNoUrl()
            ...
            _.forEach(fineDatas, function (fineData) {
                jQuery.ajax({
                    ...
                    success: function (data) {
                        ...
                        querymonitordataService.save(params).then(function (response) {
        ...
        function loadList(pageNumber, pageSize) {
            ...
            querymonitordataService.loadList(params).then(loadListSuccess);
        }
        ...
        function addData(fineData) {
            ...
            if (!fineData) {
                ...
                fineData.name = name;
                fineData.config_type = config_type;
            }
            if (fineData.name) {
                querymonitordataService.save(fineData)
        ...
        function editData(item) {
            if (!item.id) {
                AppTools.errorTips("请选择要修改的模板!");
                return;
            }
            var title = '检索条件';
            var params = {
                'fine_config_id': item.id
            };
        ...
        function opParamsNgDialog(title, params) {
            ...
            ngDialog.open({
                title: title,
                template: "priceMonitorGoods/queryAnalysisData/selectParams/querymonitordata.selectParams.html",
                height: height * 0.95 + 'px',
                width: width * 0.95 + 'px',
                controller: 'QuerymonitordataSelectParamsCtrl as sp',
                data: params,
                resolve: {
                    loadEditFiles: function ($ocLazyLoad) {
                        return $ocLazyLoad.load([
                            './priceMonitorGoods/queryAnalysisData/selectParams/querymonitordata.selectParams.controller.js',
                            './priceMonitorGoods/queryAnalysisData/selectParams/querymonitordata.selectParams.service.js',
                            'mCustomScrollbar',
                            'chosen'
                        ]);
                    }
        ...
        function is_publicData(item) {
            ...
            if (item.data_type) {
                ...
                ngDialog.open({
                    title: '选择共享人',
                    template: "priceMonitorGoods/queryAnalysisData/object_list/object_list.html",
                    height: '800px',
                    width: '800px',
                    ...
                    resolve: {
                        loadEditFiles: function ($ocLazyLoad) {
                            return $ocLazyLoad.load([
                                './priceMonitorGoods/queryAnalysisData/object_list/object_list.controller.js',
                                './priceMonitorGoods/queryAnalysisData/object_list/object_list.service.js',
                                'mCustomScrollbar'
                            ]);
                        }
                    },
        ...
        function cancelCollctData(item) {
            ...
            AppTools.confirm("确定收藏所选记录吗？", function () {
                querymonitordataService.cancelCollctData({'ids': item.id}).then(function (response) {
                    if (response.data) {
                        loadList();
        ...
        function delData(item) {
            ...
            AppTools.confirm("确定删除所选记录吗？", function () {
                querymonitordataService.delData({'ids': item.id}).then(function (response) {
                    if (response.data) {
                        loadList();
        ...
        function viewAllCollect() {
            ...
            ngDialog.open({
                title: '查看其它共享',
                template: "priceMonitorGoods/queryAnalysisData/otherShare/querymonitordata.otherShare.html",
                height: height * 0.95 + 'px',
                width: width * 0.95 + 'px',
                controller: 'QuerymonitordataOtherShareCtrl as os',
                // data: params,
                resolve: {
                    loadEditFiles: function ($ocLazyLoad) {
                        return $ocLazyLoad.load([
                            './priceMonitorGoods/queryAnalysisData/otherShare/querymonitordata.otherShare.controller.js',
                            './priceMonitorGoods/queryAnalysisData/otherShare/querymonitordata.otherShare.service.js'
                        ]);
                    }
                },
                preCloseCallback: function () {
