...
(function () {
    ...
    NoticeCtrl.$inject = ['$scope', '$stateParams', 'officeService', 'pageInfDefault', 'ngDialog', '_', '$window'];

    /* @ngInject */
    function NoticeCtrl($scope, $stateParams, officeService, pageInfDefault, ngDialog, _, $window) {
        var vm = this;
        ...
        /*默认状态*/
        vm.category_is_public = [{id: 1, chinaname: "发布"}, {id: 2, chinaname: "暂存"}, {id: 0, chinaname: "取消"}];
        activate();
        ...
        function editNotice(notice) {
            opNotice("修改消息通知", {"noticeId": notice.id});
        }
        ...
        function opNotice(title, params) {
            ngDialog.open({
                ...
                height: '700px',
                width: '800px',
                controller: 'NoticeDetailCtrl as vm',
                data: params,
                resolve: {
                    loadNoticeDetailFiles: function ($ocLazyLoad) {
                        return $ocLazyLoad.load([
                            './office/notice/detail/notice.detail.controller.js',
                            'My97DatePicker'
                        ]);
                    }
                },
