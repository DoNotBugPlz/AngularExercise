...
(function (root, factory) {
    ...
    var default_width = '768px';
    var default_height = '324px';
            ...
            function ($document, $templateCache, $compile, $q, $http, $rootScope, $timeout, $window, $controller, $injector) {
                ...
                var privateMethods = {
                    ...
                    getFocusableElements: function ($dialog) {
                        ...
                        //todo
                        /**IE8 not 选择器无法使用querySelectorAll**/
                        // var rawElements = dialogEl.querySelectorAll(focusableElementSelector);
                        var rawElements= angular.element(focusableElementSelector,dialogEl);
                ...
                var publicMethods = {
                    ...
                    open: function (opts) {
                        ...
                        $q.all({
                            ...
                            var html = [];
                            html.push('<div class="common_modal" role="document">');
                            html.push('     <div class="modal_head">');
                            html.push('         <span class="model_head_title">'+options.title+'</span>');
                            html.push('     <a href="javascript:void(0);" class="close-btn ngdialog-close" ng-click="close()"></a>');
                            html.push('     </div>');
                            html.push(template);
                            html.push("</div>");
                            template = html.join("");
                            ...
                            $dialog.html(options.overlay ?
                                '<div class="ngdialog-overlay"></div>' + template :
                                template);
                            ...
                            $dialogContent = $dialog.find('.common_modal');
                            var temp = $($dialogContent);
                            if (options.width) {
                                temp.css('width',options.width);
                            } else {
                                options.width = default_width;
                                temp.css('width',options.width);
                            }
                            if(options.isFullscreen){
                                temp.css({'left':'0px','margin-left':'0px'});
                            }else{
                                temp.css({'left':'50%','margin-left':'-' + parseInt(options.width.replace('px', '') / 2) + 'px'});
                            }

                            if (options.height) {
                                temp.css('height',options.height);
                            } else {
                                options.height = default_height;
                                temp.css('height',options.height);
                            }
                            if(options.isFullscreen){
                                temp.css({'top':'56px','margin-top':'0px'});
                            }else{
                                temp.css({'top':'50%','margin-top':'-' + parseInt(options.height.replace('px', '') / 2) + 'px'});
                            }
